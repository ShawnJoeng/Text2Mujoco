...
- Give every moving body collision geometry. Robot links, gripper palms and fingers, mobile bases, masts, carriages, forks, wheels, pressed buttons, and pulled levers each need at least one geom with a nonzero `contype`/`conaffinity`. `contype="0" conaffinity="0"` draws a body that passes through everything and reports no contact, so never use it to suppress an unwanted pair; use collision classes or `<contact><exclude>` and state which pair is excluded and why. Allow a visual-only geom only for detail inside the collidable envelope of the same body.
...
- Use direct qpos changes only for a documented task-level abstraction. Use joints, actuators, contacts, tendons, or constraints when physical force/control behavior is required. A pin does not excuse the path it drags the payload along.
