...
5. Implement every interaction point as an executable contract. Expose `list_interaction_points()`, `get_action_schema()`, `reset(seed=None)`, `step(action)`, `observe()`, and `is_success()` or an equivalent environment interface. Read [references/interaction_patterns.md](references/interaction_patterns.md) before implementing actions or markers.
...
7. Validate progressively using [references/validation_checklist.md](references/validation_checklist.md). Compile MJCF and run physics with `MUJOCO_GL=disable` before testing rendering. On macOS, use MuJoCo's `mjpython` with `MUJOCO_GL=glfw` for the native CGL path. On Linux, run EGL and OSMesa attempts in separate processes because the backend is selected when MuJoCo/OpenGL is first imported.
...
Make `environment.py` load caller-supplied model/spec paths, avoid global mutable simulator state, and keep task logic independent of viewer keyboard timing. Compile `model.xml` with `mujoco.MjModel.from_xml_path`. Save `.mjb` only after a real model compile succeeds.
