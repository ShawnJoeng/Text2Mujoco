...
class WarehouseNavigationEnv:
    ...
    def save_artifacts(self, output_dir: Path) -> dict[str, str]:
        ...
        if save_mjcf:
            xml_path = self._resolve_output_path(outputs.get("mjcf_path", "model.xml"), output_dir)
        if save_mjb:
            mjb_path = self._resolve_output_path(outputs.get("mjb_path", "model.mjb"), output_dir)
