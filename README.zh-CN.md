...
Text2MuJoCo 是一个 **Agent 技能包**，不是一个独立的自然语言编译器。它与 Codex 或 Claude Code 配合，把一段场景或任务描述转成可加载的 MuJoCo 3 包：解析物体、物理、传感器、动作顺序、成功条件和可见的交互点，然后用机器可读的报告而不是文字描述来支撑结果。[示例请求](showcase/sample_queries.json) 展示了预期的输入格式。
...
- **能失败的抓取** —— 负载由一个运行时开合的 `<equality><weld>` 携带，而不是靠改写它的 `qpos`。在空中松开它就会掉下去，这正是"手里的零件"与"台面上的零件"之所以可区分的原因。
- **穿不过去的标记** —— 任务标记是贴在所标注表面上的薄圆盘，下方一定有真实几何。因为 `<site>` 从不产生接触，一个悬在空中的自发光小球本来就是所有物体都会明显穿过去的东西。
- **可执行的交互点** —— 每个 affordance 都是一个真实 handler，带有类型化的 payload schema 和声明式依赖，并镜像进作为唯一真源的 `interaction_manifest.json`。乱序或格式错误的动作会被拒绝，而不是被默默接受。
- **证据，而不是声明** —— 每个包都自带 `physics_smoke.py` 与 `render_smoke.py`，每个展示场景都提交了由此产生的 JSON 报告、RGB-D 截图和帧归档。
...
| 层 | 由谁运行 | 拒绝什么 |
| --- | --- | --- |
| 清单 | `validate_manifests.py` | 规范与清单漂移：交互 ID、依赖顺序、位姿或标记站点不一致 |
| 静态 | `physics_smoke.py` | 编译不通过的 MJCF、清单与规范漂移、四元数或半尺寸换算错误、不可见的标记 |
| 模型审计 | `model_audit.py` | 下面全部八项缺陷：没有碰撞体、未声明质量、下垂的伺服、撕开的连杆接缝、悬空的标记、初始穿模、序列中途穿模、被碰撞掩码藏起来的穿模 |
| 初始接触 | `physics_smoke.py`、`model_audit.py` | `t=0` 时几何已经互相穿透的初始位姿 |
| 序列接触 | `physics_smoke.py`、`model_audit.py`、`sequence_contact_test.py` | 在既定序列的**任意一步**（不只是首尾）深于 1 mm 的穿模 |
| 抓取诚实性 | `physics_smoke.py` | 在空中解除焊接约束后不会掉落的负载 —— 也就是靠写 `qpos` 伪造的搬运 |
| 物理 | `physics_smoke.py` | 非有限状态、失效的执行器、未满足的任务谓词、非确定性 reset |
| 渲染 | `render_smoke.py` | 空白 RGB、非有限深度、交互前后画面无变化 |
| 归档 | `dense_archive_test.py` | GIF/TIFF 的帧数或时序与密集报告不一致 |
| 路径 | `artifact_path_test.py` | 逃出包根目录的产物、符号链接穿越 |

### 初始接触审计

初始位姿本身就已穿模，是最能躲过其他所有检查的失效模式：求解器会在预热阶段把重叠推开，之后物理、渲染和任务断言全都通过。这项审计在第一次 `mj_forward` 之后、任何 `mj_step` 之前执行，对编译得到的 `qpos0` 检查一次，对 `reset()` 之后的状态**再检查一次** —— 从常量赋值位置的 reset 可能重新引入 MJCF 里本来没有的穿模。
...
干净的初始位姿不够，干净的序列也不够。有八种截然不同的缺陷能躲过只看首尾的检查，而且每一种在镜头里看起来都是对的。[`model_audit.py`](showcase/model_audit.py) 会编译每个场景、让每个伺服保持不动、重放既定序列，并把这八项全部报成数字：

| 检查 | 拒绝什么 | 阈值 |
| --- | --- | --- |
| 碰撞几何 | 不出现在任何有效碰撞配对里的运动 geom —— 会渲染但穿过一切的连杆、货叉或按钮 | 任意 |
| 声明质量 | 受关节驱动刚体上既无 `mass` 也无 `density` 的 geom，会被默默按密度 1000 编译 | 任意 |
| 伺服保持 | 保持不住 `qpos0` 的位置伺服 —— 那个 `weight / kp` 的下垂量，在渲染中肉眼可见 | 2 mm / 1° |
| 连杆连续性 | 整段重放过程中，从受关节驱动的刚体到其最近的**已绘制**祖先之间的最大缝隙 | 5 mm |
| 标记贴地 | 悬在所标注表面上方、或埋进另一个 geom 里的标记站点 | 3 mm / 1 mm |
| 初始位姿 | 在编译得到的 `qpos0` 或 `reset()` 之后就已经重叠的 geom | 0.1 mm |
| 序列接触 | 既定序列任意一步（不只是首尾）出现的穿模 | 1 mm |
| 未声明的自穿模 | 只因为 `contype`/`conaffinity` 掩码过滤掉了该配对才得以互相穿透的 geom | 任意 |

最后一项是改变了整个架构的那一项。其他每一项衡量的是一个量；这一项衡量的是模型**有没有在这个量上说真话**。

#### 机器人需要一条针对自己的碰撞边界

这些展示场景过去把 `robot_part` 声明为 `contype="2" conaffinity="5"`。一对 geom 会碰撞的条件是 `(contype1 & conaffinity2) || (contype2 & conaffinity1)`，而 `2 & 5 == 0`，所以任意两个机器人 geom 从来没有被测试过。这看起来像一项优化 —— 短运动链不需要自碰撞 —— 但它意味着机械臂对自己没有任何边界，可以折叠着穿过自己的前臂，而且**没有任何审计能看见** ，因为被过滤掉的配对根本不产生可上报的接触。现在每个类都带 `conaffinity="7"`：
...
刚体用 `childclass="robot_part"` 选类，单个 geom 用 `class="world_part"` 选类；共享的 friction、`condim` 和 `solref` 依然生效，因为这些类是嵌在既有 default 内部而不是替换它。

把 `robot↔robot` 打开，会把看不见的自穿模变成真实接触，于是在关节处嵌套的连杆立刻互相顶起来。那些配对 —— 画在自己转动的连杆内部的铰链毂、套筒里的活塞 —— 现在被逐条列名，于是一处有意的重叠成了读者可以核对的设计决策，而不是位掩码的副作用：
...
审计用 `mujoco.mj_geomDistance` 测量，它无论配对是否被过滤都返回带符号距离，并且只在两个刚体是焊接邻居、或出现在 `model.exclude_signature` 里时才接受重叠。其他一律判失败。把掩码关掉，找出了一处此前正被掩码藏起来的真实缺陷：场景 07 里闭合的夹爪切进了它上方的前臂胶囊体，修法是收紧升降轴的上端限位，而不是加一条排除。
...
mujoco.mj_step = watched                 # 沉降循环调的是模块函数，不是 env.step
```

这里包裹的是模块级 `mj_step` 而不是环境方法，因为沉降循环和生成出来的控制器都直接调模块函数；包裹 `env.step` 会漏掉大部分仿真。用两个阈值，是因为接触求解器不是硬约束：静态位姿用 **0.1 mm**，此时任何可测量的重叠都是建模错误；整段序列用 **1.0 mm**，此时受载下的亚毫米穿透属于求解器有文档记载的柔度。当某个场景越界时，要改的是模型或控制器 —— 绝不是阈值。叉车的 `fork_lift` 伺服是最清楚的例子：一个刚性位置执行器收到 `0.18 m` 的阶跃指令，把货叉加速到约 `1.7 m/s`，撞进托盘底板 `4.49 mm`，因此该指令现在按大约 `0.25 m/s` 匀速给出。

#### 关节是一个坐标，不是一个形状

有三种彼此独立的缺陷渲染出来一模一样 —— 手从手臂上掉下来了 —— 而它们都不是拓扑错误。最严重的是没有被画出来的移动轴：一个滑动关节，它的移动刚体带几何，但它的**行程**不带几何，于是在父连杆下方裂开一道随关节量增大的缝。在完全伸出时，场景 07 的工具悬在前臂下方 165 mm，中间什么都没有，而且没有任何接触、位姿或任务断言看得见，因为运动链全程是完整的。现在这条轴是一个自己的刚体，带两个始终重叠的 geom：
...
那些悬在工作台上方的彩色小球是 `<site>` 元素。site 从不产生接触 —— 这是设计如此，改不了 —— 所以每个负载、每根机器人连杆都会明显地直接穿过去。把标记建模成观看者预期是实心的形状，本身就是那个 bug；修法就是不再这么做。现在每个面向观看者的标记都是一个薄圆柱，贴在它所标注的表面上，半高等于它离该表面的高度，于是底面正好齐平：

```xml
<site name="insertion_marker" type="cylinder" pos="0.17 0.21 0.906" size="0.050 0.006" material="marker_magenta" group="2"/>
```

审计向下投射射线以确认下方有真实几何，因此贴在机器人根本到不了的空地上的贴花会失败；它也拒绝中心被埋进另一个 geom 的情况，因此贴花必须避开它所标注负载的落地范围。代码当作运动学参考读取的 site —— 工具中心、叉尖 —— 移到了 `group="4"`，根本不绘制。没有任何东西引用的 site 被删除。八个场景中这样重新归类了 51 个 site：其中 31 个是悬空或被埋的，若干个只是相机位置的副本，场景 03 里还有三个是沉进地板 15 mm 的小球。

#### 抓取是一个约束，不是一次坐标写入

每个展示场景过去都通过每步改写自由关节的 `qpos` 来搬运负载。没有任何情况能让负载滑脱，松手是传送到一个硬编码常量，而且 —— 这正是被反馈的那个缺陷 —— 躺在台面上的零件与手里的零件在状态上**无法区分**，于是掉落的零件一直被当成正在被操作的对象。搬运现在是一个相等约束，声明时不激活，运行时开合：
...
每个场景都附带一段**按 MuJoCo 仿真时间每 `0.20 s` 采样**的密集序列 —— 不是墙钟时间，也不是每个渲染 tick。采样器保留每个 `0.20 s` 边界处（或之后）的第一个步后状态，并在每个动作边界额外加一帧事件帧，因此节奏保持精确，同时不漏掉任何重要瞬间。
...
**已验证 —— PASS。** 方块落进五面开口盒内部，与盒底接触，最终静止在 `4.9e-14 m/s`，并在相机图像中移动 `130.36 px`。在空中释放时方块掉落 `225 mm`，说明搬运是一个约束，而不是一次 `qpos` 写入。
...
- **接触审计** —— 机器人的转塔和朝向指示块属于它可碰撞的外壳，而不是装饰；3,375 步审计中没有可测量的穿模。间隙是靠路径保持的，而不是靠碰不到任何东西的几何。
...
- **同步** —— 托盘由 `fork_carriage` 与托盘之间的 `pallet_grasp` 焊接携带，偏移取自货叉插入到位时的实测值。对叉车来说，诚实性测试的正确形式是滑移而不是空中掉落：焊接时托盘保持在 `0.303 mm` 以内，释放后滑移 `73.287 mm`，下限是 `20 mm`。
...
- **顺序** —— `lower_forks_release` 先把托盘放到配送台上，**然后**才降下空货叉，并在 `0.045 m` 升降高度处退出 —— 位于通道中部，下方距台面、上方距托盘底板各有 `40 mm` 余量。配送目标是带定位块的平台而不是带围墙的料箱，因为叉车必须在板面高度把叉齿倒着抽出来，而接近侧一道 `0.24 m` 的围墙是既定序列无法越过的几何。
- **接触审计** —— 5,386 步审计中最深穿模 `0.4821 mm`。走到这一步靠的是两处真实修复，而不是放宽阈值：`fork_carriage_geom` 没写 `mass` 属性，于是按 MuJoCo 默认密度重达 `28.7 kg`，任何 `kp=300` 的伺服都托不住 —— 插销一松，叉齿立刻掉到关节限位。改成显式质量并用 `kp=6000 kv=300` 之后，一个阶跃指令又把叉齿砸进托盘底板 `4.49 mm`，因此 `_set_lift` 现在按大约 `0.25 m/s` 匀速给出。
...
**渲染后端。** 物理完全不需要 GPU，也不需要 GL 上下文 —— `MUJOCO_GL=disable` 才是 `physics_smoke.py` 的正确设置。渲染方面，macOS 用 `mjpython` 配 `MUJOCO_GL=glfw`，这会把原生 CGL 上下文交给 MuJoCo —— 每份已提交的展示报告都把自己的 `renderer_context` 记录为经由 glfw 后端的 CGL，而不是软件光栅化器。在无头 Linux 上优先用 `MUJOCO_GL=egl`，并在**另一个进程**里退回 `MUJOCO_GL=osmesa`，因为后端是在 MuJoCo 首次导入 OpenGL 时选定的。不要把 OSMesa 的图像标称为 GPU 渲染。
